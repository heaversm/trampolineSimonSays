...
const int leds[3] = { LEDPIN1, LEDPIN2, LEDPIN3 };
...
const int PIEZO_PIN1 = 8; // pin on which we read vibration / piezo output
const int PIEZO_PIN2 = 9; // pin on which we read vibration / piezo output
const int PIEZO_PIN3 = 10; // pin on which we read vibration / piezo output
//.const int piezos[4] = { PIEZO_PIN0, PIEZO_PIN1, PIEZO_PIN2, PIEZO_PIN3 };
const int piezos[3] = { PIEZO_PIN1, PIEZO_PIN2, PIEZO_PIN3 };
...
const int WIN_SOUND_PIN = 5; //old: 3

const int NUMPIXELS1 = 172; //how many lights are on our LED strip?
const int NUMPIXELS2 = 172; //how many lights are on our LED strip?
const int NUMPIXELS3 = 172; //how many lights are on our LED strip?

 //initialize neopixels
//Adafruit_NeoPixel pixels0(NUMPIXELS, LEDPIN0, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel pixels1(NUMPIXELS1, LEDPIN1, NEO_GRB + NEO_KHZ800); 
Adafruit_NeoPixel pixels2(NUMPIXELS2, LEDPIN2, NEO_GRB + NEO_KHZ800);
Adafruit_NeoPixel pixels3(NUMPIXELS3, LEDPIN3, NEO_GRB + NEO_KHZ800);

//Adafruit_NeoPixel pixelsArray[4] = { pixels0,pixels1,pixels2,pixels3 };
Adafruit_NeoPixel pixelsArray[3] = { pixels1,pixels2,pixels3 };


//define the colors we want to cycle through
uint32_t colorRed = pixels1.Color(255, 0, 0);
uint32_t colorGreen = pixels1.Color(0, 150, 0);
uint32_t colorBlue = pixels1.Color(0, 255, 255);
uint32_t colorYellow = pixels1.Color(255, 255, 0);
...
int totalNumColors = 3; //number of colors user can cycle through
uint32_t colorArray[3] = {colorGreen, colorBlue, colorYellow};
...
void handleBounceMode() {
  ...
  for (int i = 0; i < totalNumColors; i++) {
    ...
    if (piezoV == HIGH) { //if the vibration is significant enough to constitute a bounce, and we haven't already triggered a bounce...
      ...
      Serial.println(piezoV);
      pixels[i].fill(curPixelColor, 0, NUMPIXELS);
      pixels[i].show();
